...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Seed bills — guarded: only runs when the table is completely empty.
    // Without a unique index on name (dropped in Migration 007), ON CONFLICT DO NOTHING
    // is a no-op, so re-running the migration would re-insert deleted bills.
    // The count guard prevents that.
    const [{ count: billCount }] = await sql`SELECT COUNT(*)::int AS count FROM finance_bills` as [{ count: number }]
    if (billCount === 0) {
      await sql`
    ...
    // --- Migration 008: credit_amount for statement credits on bills + annual items ---
    await sql`ALTER TABLE finance_bills ADD COLUMN IF NOT EXISTS credit_amount NUMERIC(10,2)`
    await sql`ALTER TABLE finance_annual_items ADD COLUMN IF NOT EXISTS credit_amount NUMERIC(10,2)`

    // --- Migration 009: per-month credit on actuals ---
    await sql`ALTER TABLE finance_actuals ADD COLUMN IF NOT EXISTS credit_amount NUMERIC(10,2)`
